...
const serveSaveItem = function (request, response, next) {
  ...
  const { title, item, statusCode } = JSON.parse(request.body);
  const id = new Date().getTime();
  previousTodo[title].push({ id, item, statusCode });
...
const serveToggleStatus = (request, response, next) => {
  const previousTodo = JSON.parse(fs.readFileSync(DATA_STORE, 'utf8'));
  const { title, statusCode, id } = JSON.parse(request.body);
  const statusToChange = previousTodo[title].find(obj => {
    return obj.id === +id;
  });
  const index = previousTodo[title].indexOf(statusToChange);
  previousTodo[title][index].statusCode = statusCode;
  fs.writeFileSync(DATA_STORE, JSON.stringify(previousTodo));
  response.end();
  next();
};
