const { App } = require('./app');
const {
  readBody,
  todoData,
  serveStaticPage,
  pageNotFound,
  serveSaveTitle,
  serveDeleteItem,
  serveToggleStatus,
  serveChangeTitle,
  methodNotAllowed,
  serveSaveItem,
  serveChangeItem,
  serveMatch,
  serveDeleteTitle
} = require('./handlers');


// const assignPostHandlers = () => {
//   const postHandlers = {
//     '/saveTitle': serveSaveTitle,
//     '/saveItem': serveSaveItem,
//     '/toggleStatus': serveToggleStatus,
//     '/deleteItem': serveDeleteItem,
//     '/deleteTitle': serveDeleteTitle,
//     '/changeTitle': serveChangeTitle,
//     '/changeItem': serveChangeItem,
//     '/findGivenContent': serveMatch
//   };

//   for (const key in postHandlers) {
//     app.post(key, postHandlers[key]);
//   }
// }


const app = new App();

app.use(readBody);
app.get('/todoList.json', todoData);
app.get('', serveStaticPage);
app.get('', pageNotFound);
app.post('/saveTitle', serveSaveTitle);
app.post('/saveItem', serveSaveItem);
app.post('/toggleStatus', serveToggleStatus);
app.post('/deleteItem', serveDeleteItem);
app.post('/deleteTitle', serveDeleteTitle);
app.post('/changeTitle', serveChangeTitle)
app.post('/changeItem', serveChangeItem);
app.post('/findGivenContent', serveMatch);
app.use(methodNotAllowed);

module.exports = { app };
